...
pacman::p_load(readxl, dplyr, plotrix, ggplot2, forcats, Hmisc, sciplot, plyr)
```
...
Usando o peso dos recém-nascidos(`pesoRN`) e o `sexo` do banco de dados `RNT`, vamos clacular a média e o desvio padrão por sexo, usando o pacote `plyr`:

```{r}
media <- dados %>% 
  ddply("sexo", summarise, medias = mean(pesoRN))
...
ggplot(dados, aes(x=pesoRN, fill=sexo)) +
  ...
  scale_fill_manual(values=c("lightskyblue1", "pink2")) +
  theme_classic() +
  geom_vline(data=media, aes(xintercept=medias, color=sexo),
             linetype="dashed") +
  xlab("Peso do Recém-nascido (g)") +
  ylab ("Frequência")
```
